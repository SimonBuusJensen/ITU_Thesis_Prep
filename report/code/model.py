...
class CNNModel:
    ...
    def cnn_model(x):
        # Convolution Layer 1
        conv1_layer = tf.nn.conv1d(value=x, filters=weights['wc1'],
                                   stride=1, padding='SAME')
        conv1_layer = tf.nn.bias_add(conv1_layer, biases['bc1'])
        conv1_layer = tf.nn.relu(conv1_layer)

        # Convolution Layer 2
        conv2_layer = tf.nn.conv1d(value=conv1_layer, filters=weights['wc2'],
                                   stride=1, padding='SAME')
        conv2_layer = tf.nn.bias_add(conv2_layer, biases['bc2'])
        conv2_layer = tf.nn.relu(conv2_layer)

        # Convolution Layer 3
        conv3_layer = tf.nn.conv1d(value=conv2_layer, filters=weights['wc3'],
                                   stride=1, padding='SAME')
    ...
    # Weight & Bias of the model
    weights = {
        # A 1D convolution of size 11 with 44 input- and 88 outputs channels
        'wc1': tf.get_variable(name='wc1', shape=[11, 44, 88], dtype=tf.float32,
                               initializer=tf.contrib.layers.xavier_initializer()),
        # A 1D convolution of size 11 with 88 input- and 44 outputs channels
        'wc2': tf.get_variable(name='wc2', shape=[11, 88, 44], dtype=tf.float32,
                               initializer=tf.contrib.layers.xavier_initializer()),
        # A 1D convolution of size 11 with 44 input- and 4 outputs channels
        'wc3': tf.get_variable(name='wc3', shape=[11, 44, 4], dtype=tf.float32,
                               initializer=tf.contrib.layers.xavier_initializer()),
    }
    biases = {
        'bc1': tf.get_variable(name='bc1', shape=[88], dtype=tf.float32,
    ...
    , x_test, y_test, x_valid, y_valid, train_end_idx, test_end_idx, valid_end_idx):
    ...
    def train(x_train, y_train):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(50):
                # Shuffling train data before each epoch
                ...
                for i in range(n_train_batches):
                    ...
                    batch_loss, _ = sess.run([loss_function, optimizer],
